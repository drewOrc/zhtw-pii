// Corrected re-measurement of ckip-bert-tiny-ner on mobile-fast3g. The main
// run (scripts/run_benchmark.mjs) used a 180s timeout that undercounted the
// WASM runtime download (see RESULTS.md "A timeout that needed correcting
// mid-run") and all 3 reps timed out. This script reruns just that one
// (model, network condition) cell with the corrected 300s timeout. Writes
// results/s1_results_bert_tiny_fast3g_fixup.json, merged into the final
// results/s1_results.json by scripts/merge_results.mjs.
//
// Run with: node scripts/run_bert_tiny_fast3g_fixup.mjs
...
async function main() {
  ...
  fs.writeFileSync(
    path.join(RESULTS_DIR, "s1_results_bert_tiny_fast3g_fixup.json"),
    JSON.stringify(out, null, 2)
  );
  console.log("Wrote results/s1_results_bert_tiny_fast3g_fixup.json");
}
